    ...
    >
      ...
      <div style={styles.splitRow}>
        ...
        <div style={styles.splitPane}>
          ...
          {prevInd && prevInd.metrics && prevInd.metrics.length > 0 ? (
            prevInd.metrics.map((m, i) => (
              <div key={i} style={styles.metricItem}>
                <MetricChart metric={m} />
              </div>
            ))
          ) : (
            <div style={styles.noData}>직전 년도 데이터가 없습니다.</div>
          )}
        </div>
        ...
        <div style={styles.splitPane}>
          ...
          {ind.metrics.length === 0 ? (
            <div style={styles.noData}>연결된 데이터가 없습니다.</div>
          ) : (
            ind.metrics.map((m, i) => (
              <div key={i} style={styles.metricItem}>
                <MetricChart metric={m} />
...
const styles = {
  ...
  metricItem: {
    paddingBottom: 16,
    borderBottom: '1px dashed #e3dbbb',
    marginBottom: 4,
  },
  noData: {
    color: '#94A3B8',
