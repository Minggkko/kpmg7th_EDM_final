...
logging.basicConfig(
    ...
    force=True,  # uvicorn이 먼저 설정해도 덮어씀
)
from app.core.dependencies import get_current_user
from app.api.v1 import issues, indicators, data_points, data, mapping, raw_data, auth
from app.api.v1 import outliers, evidence, finalization, dashboard, audit, aggregation, sites
...
app.include_router(aggregation.router,  prefix="/api/v1/aggregation",  tags=["Aggregation"],  dependencies=[Depends(get_current_user)])
app.include_router(sites.router,        prefix="/api/v1/sites",        tags=["Sites"],        dependencies=[Depends(get_current_user)])
