...
def log_action(
    ...
    ----------
    std_id : int
        변경 대상 standardized_data 레코드의 id
    action : str
...
def get_audit_logs(
    ...
    site_id : str, optional
        사업장 필터 (standardized_data와 조인하여 필터링)
    action : str, optional
    ...
    try:
        ...
        # site_id 필터: audit_trail에 site_id가 없으므로 std_id → standardized_data 매칭
        if site_id and logs:
...
def _get_std_ids_for_site(client, site_id: str) -> set[int]:
    """site_id에 속하는 standardized_data id 집합 반환"""
    result = (
