...
def get_verification_dashboard(
    ...
    # ── Step 1: standard_usage 조회 (필터 적용) ──────────────────────────────
    query = client.table("standard_usage").select("*").order("reporting_date", desc=True).limit(limit)
...
def get_status_summary(site_id: str = None) -> list[dict]:
    ...
    query = client.table("standard_usage").select("site_id, v_status")
    if site_id:
        query = query.eq("site_id", site_id)
