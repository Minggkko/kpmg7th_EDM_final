...
Step 2. standard_usage 업데이트 (original_value 백업 + value 보정 + v_status=5)
...
def finalize_usage_data(
    ...
    try:
        client.table("standard_usage").update(update_payload_full).eq("id", std_id).execute()
        update_payload = update_payload_full
    except Exception:
        # 확장 컬럼 없을 경우 핵심 컬럼만 업데이트 (마이그레이션 전 호환)
        try:
            client.table("standard_usage").update(update_payload_minimal).eq("id", std_id).execute()
            update_payload = update_payload_minimal
            print("⚠️  확장 컬럼 미존재 → 핵심 컬럼(value, v_status)만 업데이트 (마이그레이션 후 전체 적용)")
            ...
            "audit_trail_id": None,
            "message": f"standard_usage 업데이트 실패 (DB 변경 없음): {e}",
        }
    ...
    if audit_result["status"] == "error":
        print(f"⚠️  audit_trail 오류. standard_usage 롤백을 시도합니다.")
        rollback_payload = {"value": float(before_value), "v_status": before_status}
        try:
            client.table("standard_usage").update(rollback_payload).eq("id", std_id).execute()
            return {
...
def revert_finalization(std_id: int, user_id: str, reason: str) -> dict:
    ...
    try:
        client.table("standard_usage").update({
