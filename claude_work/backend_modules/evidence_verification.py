...
evidence_usage 와 standard_usage 를 비교하여 gap_percent 를 계산하고
verification_logs 에 기록하며 v_status 를 자동 전이시킵니다.
...
---------
evidence_usage (미검증)
    ↔ standard_usage (site_id + metric_name + reporting_date)
    → gap_value / gap_percent 계산
    → result_code / unit_mismatch 판정
    → verification_logs INSERT
    → standard_usage.v_status UPDATE (1 → 3 / 4 / 5)
...
def verify_evidence_data(
    ...
    """
    evidence_usage 와 standard_usage 를 비교하여 정합성을 검증합니다.

    아직 verification_logs 에 없는 evidence_usage 레코드를 대상으로
    - verification_logs INSERT
    - standard_usage.v_status UPDATE (v_status=1 인 경우만)
    - raw_ocr_data.processing_status = "Success"
    - audit_trail 기록
    을 수행합니다.
    ...
    try:
        ...
        # ── STEP 2: 이미 검증된 evidence_id 조회 (중복 방지) ──────────────
        verified_ids_rows = (
        ...
        already_verified: set[int] = {
            r["evidence_id"] for r in verified_ids_rows if r.get("evidence_id")
        }
        ...
        # ── STEP 3: standard_usage 전체 캐시 (Python-side 매칭용) ─────────
        std_rows = client.table("standard_usage").select("*").execute().data
        ...
        for ev in pending_evidence:
            ...
            if not std_record:
                logger.warning(
                    f"[evidence_verification] standard_usage 매칭 없음 "
