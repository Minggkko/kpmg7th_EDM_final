...
# .env 탐색 (database_utils 와 동일한 경로 우선순위)
_ROOT = Path(__file__).resolve().parent.parent.parent
for _p in [_ROOT / ".env", _ROOT / "DB_postgresql" / ".env"]:
    if _p.exists():
...
def analyze_outlier_with_llm(outlier_id: int = None) -> dict:
    ...
    try:
        ...
        for item in targets:
            o_id     = item["id"]
            std_data = item.get("standard_usage")
            if not std_data:
                logger.warning(f"[outlier_llm] outlier_id={o_id}: standard_usage 없음 → 스킵")
                continue
