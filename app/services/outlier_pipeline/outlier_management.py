...
v_status 소명 대상
------------------
  2 : PASS + 불일치 (이상치 없음, 증빙과 다름)  → 확인 요청 또는 소명 입력
  3 : FAIL + 일치   (이상치 탐지, 증빙과 같음)  → 수정 금지, 이상치 원인 소명 입력
  4 : FAIL + 불일치 (이상치 탐지, 증빙과 다름)  → 소명 이유 입력 + 수정 가능

상태 전이
---------
  action_taken != '확인요청'  → v_status = 5 (Verified, 소명 완료)
  action_taken == '확인요청'  → v_status 유지 (담당자 회신 대기 중)
  ※ 값 수정이 필요한 정정은 /finalization 엔드포인트를 사용하세요.
"""
...
# ── 상수 ──────────────────────────────────────────────────────────────────────
V_STATUS_VERIFIED    = 5   # 소명/확정 완료 (Verified)
V_STATUS_PASS_MISS   = 2   # PASS + 불일치 (이상치 없음, 증빙과 다름)
V_STATUS_FAIL_MATCH  = 3   # FAIL + 일치   (이상치 탐지, 증빙과 같음)
V_STATUS_FAIL_MISS   = 4   # FAIL + 불일치 (이상치 탐지, 증빙과 다름)

# 소명이 필요한 v_status 집합
JUSTIFICATION_TARGETS = {V_STATUS_PASS_MISS, V_STATUS_FAIL_MATCH, V_STATUS_FAIL_MISS}
...
def update_outlier_justification(
    ...
    ----------
    std_id : int
        소명 대상 standardized_data 레코드의 id
    user_feedback : str
    ...
    # ── Step 1: 현재 standardized_data 레코드 확인 ───────────────────────────────
    ...
    # 소명 대상 v_status(2/3/4)인지 확인
    if before_status not in JUSTIFICATION_TARGETS:
        raise ValueError(
            f"v_status={before_status} 레코드는 소명 대상이 아닙니다. "
            f"소명 가능 상태: {sorted(JUSTIFICATION_TARGETS)}"
        )
    ...
    justification_record = {
        ...
        "created_at": now,
        "resolved_at": now if action_taken != "확인요청" else None,
    }
    ...
    # action_taken != '확인요청' : 소명 완료 → v_status=5 (Verified)
    # action_taken == '확인요청' : 담당자 회신 대기 → v_status 유지
    v_status_changed = False
    new_v_status = before_status

    if action_taken != "확인요청":
        new_v_status = V_STATUS_VERIFIED
        v_status_changed = True

        try:
            # standardized_data v_status 업데이트
            client.table("standardized_data").update({"v_status": new_v_status}).eq("id", std_id).execute()
        except Exception as e:
            return {
    ...
    # outlier_results.is_resolved → 소명 완료 여부와 무관하게 항상 True 마킹
    if outlier_id:
        try:
            client.table("outlier_results").update({"is_resolved": True}).eq("id", outlier_id).execute()
        except Exception as e:
            pass  # is_resolved 업데이트 실패는 소명 자체를 무효화하지 않음
...
def get_outlier_detail(std_id: int) -> dict:
    """
    특정 standardized_data 레코드의 이상치 상세 정보를 통합 반환합니다.
