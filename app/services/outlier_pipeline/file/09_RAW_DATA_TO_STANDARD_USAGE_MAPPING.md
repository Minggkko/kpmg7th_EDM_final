# 📊 raw_data → standardized_data 직접 추출

**목적:** 팀원이 이해하고 로직을 만들기 위한 데이터 흐름 및 테이블 구조  
**대상:** raw_data 테이블 + data_points 테이블 → standardized_data 테이블
...
## 📤 **2단계: 출력 데이터 (standardized_data)**

### **standardized_data 테이블 구조 (최종 버전)**
...
규칙:
  ...
  4. 저장:
     - standardized_data.value = 1200.0 (DOUBLE PRECISION)
     - standardized_data.raw_value_text = "1,200" (원본 TEXT 보존)
...
| standardized_data 컬럼 | 타입 | 필수 | 출처 | 변환 규칙 |
|-------------------|------|------|------|---------|
...
✅ 모두 만족 → standardized_data INSERT
❌ 하나라도 불만족 → 스킵하고 로그 기록
```
...
□ 검증 쿼리 준비
  └─ raw_data 행 수 vs standardized_data v_status=0 행 수 비교
  └─ NULL 체크
  └─ 값 범위 체크
```
