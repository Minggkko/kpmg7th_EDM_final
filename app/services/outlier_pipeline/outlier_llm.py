...
# ── 시스템 프롬프트: v_status=3/4 (이상치 탐지 항목) ─────────────────────────
_SYSTEM_INSTRUCTION_OUTLIER = """
...
# ── 시스템 프롬프트: v_status=2 (증빙 불일치, 이상치 탐지 없음) ──────────────
_SYSTEM_INSTRUCTION_MISMATCH = """
너는 ESG 보고 데이터와 증빙 자료 간의 불일치를 분석하는 'ESG 데이터 검증 전문가'이다.
이 항목은 통계적 이상치 탐지는 통과했으나, 시스템 입력값과 OCR 증빙값이 일치하지 않는다.

[분석 관점]
- 수동 입력 오류: 전표 기입 실수, 단위 혼동(kWh/MWh, Nm3/kNm3 등)
- OCR 인식 오류: 문서 스캔 품질 불량, 글자 오인식
- 보고 기간 불일치: 증빙 문서와 보고 기간이 다른 경우
- 버전 차이: 수정된 증빙과 입력된 수치의 버전 불일치

[진단 및 해설 원칙]
1. 두 값의 차이와 오차율을 명확히 제시할 것.
2. 어느 값이 실제 값일 가능성이 높은지 판단 근거를 제시할 것.
3. 담당자가 즉시 실행할 수 있는 확인 절차를 체크리스트로 제시할 것.
"""
...
"""
    ...
    threshold_str = str(threshold) if threshold is not None and threshold > 0 else "해당없음"
    return f"""
    [진단 대상 데이터]
    - 사업장: {site} ({date})
    - 지표: {metric_name}
    - 측정값: {value}
    - 시스템 임계치: {threshold_str}
    - 탐지 계층: {layer}
...
def analyze_outlier_with_llm(outlier_id: int = None) -> dict:
    ...
    try:
        ...
        # ── STEP 1: 진단 대상 조회 ─────────────────────────────────────────
        # (A) outlier_results 에서 analysis_summary IS NULL 인 항목 (v_status 3, 4)
        query = (
        ...
        # (B) v_status=2 항목 추가 처리 (증빙불일치, 이상치 탐지 없음)
        #     outlier_results 레코드가 아예 없어서 LLM 진단을 받지 못하는 경우를 처리.
        #     해당 std_id에 대해 outlier_results 레코드가 없으면 최소 레코드를 생성하여 대상에 포함.
        if outlier_id is None:
            existing_std_ids: set[int] = {
                int(r["std_id"]) for r in
                client.table("outlier_results").select("std_id").execute().data
                if r.get("std_id") is not None
            }
            v2_rows = (
                client.table("standardized_data")
                .select("id, site_id, metric_name, value, reporting_date, v_status")
                .eq("v_status", 2)
                .execute()
                .data
            )
            for v2 in v2_rows:
                if int(v2["id"]) not in existing_std_ids:
                    try:
                        res = client.table("outlier_results").insert({
                            "std_id"        : int(v2["id"]),
                            "layer"         : "증빙불일치",
                            "detected_value": float(v2["value"]),
                            "threshold"     : None,
                            "severity"      : "Warning",
                        }).execute()
                        if res.data:
                            targets.append(res.data[0])
                            logger.info(
                                f"[outlier_llm] v_status=2 outlier_results 생성: std_id={v2['id']}"
                            )
                    except Exception as e:
                        logger.warning(
                            f"[outlier_llm] v_status=2 outlier_results INSERT 실패 std_id={v2['id']}: {e}"
                        )
        ...
        if not targets:
            return {
                "status" : "success",
                "data"   : [],
                "message": "진단할 새로운 데이터가 없습니다. (v_status 2/3/4 항목 없음)",
                "count"  : 0,
                "error_count": 0,
            }
        ...
        for item in targets:
            ...
            # ※ verified_at 컬럼 미존재 → id(log_id) 기준 최신 1건
            vlog = (
            ...
            # v_status=2(증빙불일치): 이상치 레이어 설명 불필요 → 전용 프롬프트 사용
            system_instruction = (
                _SYSTEM_INSTRUCTION_MISMATCH if v_status == 2
                else _SYSTEM_INSTRUCTION_OUTLIER
            )
            try:
                ...
                analysis_json_raw = _parse_llm_response(response.choices[0].message.content)
            except Exception as e:
                err_msg = str(e)[:300]
                logger.error(f"[outlier_llm] GPT-4o API 오류 outlier_id={o_id}: {err_msg}")
                error_count += 1
                # analysis_summary 를 NULL 유지(재시도 가능)하고 다음 항목으로
                continue
        ...
        count = len(results)
        status = "success" if error_count == 0 else ("error" if count == 0 else "partial")
        message = f"{count}건 AI 진단 완료"
        if error_count > 0:
            message += f" ({error_count}건 GPT-4o 호출 실패 — API 키 또는 네트워크 확인 필요)"
        return {
            "status"     : status,
            "data"       : results,
            "message"    : message,
