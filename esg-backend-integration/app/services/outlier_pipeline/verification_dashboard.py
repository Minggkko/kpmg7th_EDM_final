...
# ── v_status 레이블 매핑 ──────────────────────────────────────────────────────
# 0 : 표준화 대기 (Pending)
# 1 : 표준화 완료 → 검증 파이프라인 입력점
# 2 : PASS + 불일치 (이상치 없음, 증빙과 다름)
# 3 : FAIL + 일치   (이상치 탐지됨, 증빙과는 같음)
# 4 : FAIL + 불일치 (이상치 탐지됨, 증빙과도 다름)
# 5 : Verified      (최종 확정 완료)
V_STATUS_LABEL = {
    0: "Pending",
    1: "Standardized",
...
def get_verification_dashboard(
    ...
    각 standardized_data 레코드에 이상치 탐지 결과, 증빙 검증 로그,
    ...
    # activity_data: site_id + reporting_date 매칭용 딕셔너리
    site_dates = list({(r["site_id"], r["reporting_date"]) for r in usage_list})
...
def get_status_summary(site_id: str = None) -> list[dict]:
    ...
    query = client.table("standardized_data").select("site_id, v_status")
    if site_id:
        query = query.eq("site_id", site_id)
