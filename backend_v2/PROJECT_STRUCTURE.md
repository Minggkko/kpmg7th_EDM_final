...
| 값 | 의미 | 설정 주체 |
|---|------|----------|
| `1` | 표준화 완료 → 이상치 탐지 대기 | `mapping_service.run_mapping()` |
| `2` | PASS + 불일치 (이상치 없음, 증빙값과 차이 있음) | `evidence_verification.verify_evidence_data()` |
| `3` | FAIL + 일치 (이상치 있음, 증빙값과 일치) | `evidence_verification.verify_evidence_data()` |
| `4` | FAIL + 불일치 (이상치 있음, 증빙값과도 차이 있음) | `evidence_verification.verify_evidence_data()` |
| `5` | 최종 확정 완료 | `data_finalization.finalize_usage_data()` |
| `99` | 단위 변환 실패 (파이프라인 제외) | `mapping_service.run_mapping()` |

> `detect_outliers()` 는 v_status 를 변경하지 않습니다. 탐지 결과를 `outlier_results` 에 기록하고
> `verify_evidence_data()` 를 자동 호출하여 최종 v_status(2/3/4/5)를 결정합니다.
