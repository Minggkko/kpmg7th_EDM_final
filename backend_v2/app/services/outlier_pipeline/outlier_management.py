...
v_status 소명 대상
------------------
  2 : PASS + 불일치 (이상치 없음, 증빙과 다름)  → 사용자가 수정/유지 선택 + 이유 입력
  3 : FAIL + 일치   (이상치 탐지, 증빙과 같음)  → 수정 금지, 이상치 원인 소명 입력
  4 : FAIL + 불일치 (이상치 탐지, 증빙과 다름)  → 소명 이유 입력 (OCR값 자동 수정)

상태 전이
---------
  action_taken == '정상'  → v_status = 1 (Normal, 검증 통과로 간주)
  action_taken != '정상'  → v_status 유지 (최종 확정은 /finalization 에서 처리)
"""
...
def update_outlier_justification(
    ...
    # action_taken == '정상' : 이 값이 정상임을 확인 → v_status=1 (Normal)
    # 그 외                   : v_status 유지 (최종 확정은 /finalization 에서)
    v_status_changed = False
    new_v_status = before_status
