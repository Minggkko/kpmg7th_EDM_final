    ...
    for section in draft["sections"]:
        ...
        for item in section["items"]:
            ...
            for dp in item["data_points"]:
                if not dp["has_data"]:
                    story.append(Paragraph(
                        f"{dp['dp_name']} — 입력된 데이터가 없습니다.",
                        style_nd
                    ))
                    continue

                story.append(Paragraph(
                    f"<b>{dp['dp_name']}</b>  (단위: {dp['unit']})",
                    style_p
                ))

                chart_type = dp.get("chart_type", "table")
                if chart_type in ("bar", "line", "donut", "breakdown_bar"):
                    from reportlab.platypus import Image as RLImage
                    img_buf = _render_chart_image(dp)
                    story.append(RLImage(img_buf, width=140 * mm, height=65 * mm))
                else:
    ...
    for section in draft["sections"]:
        ...
        for item in section["items"]:
            ...
            for dp in item["data_points"]:
                if not dp["has_data"]:
                    p   = doc.add_paragraph()
                    run = p.add_run(f"{dp['dp_name']} — 입력된 데이터가 없습니다.")
                    run.font.color.rgb = RGBColor(0xA0, 0xAE, 0xC0)
                    run.font.size      = Pt(9)
                    continue

                label = doc.add_paragraph()
                run   = label.add_run(f"{dp['dp_name']}  (단위: {dp['unit']})")
                run.bold      = True
                run.font.size = Pt(9)
...
# ═══════════════════════════════════════════════════════════════════════════════
# 차트 이미지 렌더링 (matplotlib)
# ═══════════════════════════════════════════════════════════════════════════════

_CHART_COLORS = ['#2563EB', '#10B981', '#F59E0B', '#EF4444', '#8B5CF6']
...
def _render_chart_image(dp: dict, w: float = 5.5, h: float = 2.8) -> io.BytesIO:
    """
    dp의 chart_type과 aggregated 데이터를 바탕으로 matplotlib 차트를
    PNG BytesIO로 렌더링합니다.

    chart_type:
        bar / line      → 월별 꺾은선/막대 (사이트별 색상 구분)
        breakdown_bar   → 연간 합계 가로 막대
        donut           → 도넛 차트 (사이트별)
        table / text    → 이 함수를 호출하지 않음 (호출자 책임)
    """
    import matplotlib
    matplotlib.use("Agg")
    ...
    chart_type = dp.get("chart_type", "table")
    aggregated = dp.get("aggregated", {})
    unit       = dp.get("unit", "")
    title      = f"{dp['dp_name']}  ({unit})" if unit else dp["dp_name"]

    # ── 도넛 ──
    if chart_type == "donut":
        by_site = aggregated.get("by_site", {})
        sites   = list(by_site.keys())
        n       = max(len(sites), 1)
        fig, axes = plt.subplots(1, n, figsize=(max(w, n * 2.4), h))
        if n == 1:
            axes = [axes]
        for i, site in enumerate(sites):
            val = float(by_site[site])
            rem = max(0.0, 100.0 - val)
            axes[i].pie(
                [val, rem],
                colors=[_CHART_COLORS[i % len(_CHART_COLORS)], "#E2E8F0"],
                startangle=90,
    ...
    # ── 가로 막대 (분류별) ──
    elif chart_type == "breakdown_bar":
        by_site = aggregated.get("by_site", {})
        labels  = list(by_site.keys())
        values  = [float(v) for v in by_site.values()]
        fig, ax = plt.subplots(figsize=(w, max(h, len(labels) * 0.55 + 0.8)))
        bars = ax.barh(
            range(len(labels)), values,
            color=[_CHART_COLORS[i % len(_CHART_COLORS)] for i in range(len(values))],
            height=0.5,
        )
    ...
    # ── 세로 막대 / 꺾은선 ──
        ...
        months          = aggregated.get("months", [])
        monthly_by_site = aggregated.get("monthly_by_site", {})
        sites           = list(monthly_by_site.keys())
        ...
        if chart_type == "line":
            for i, site in enumerate(sites):
                vals = [monthly_by_site[site].get(m) or 0 for m in months]
                ax.plot(x, vals, marker="o", markersize=3,
                        label=site, color=_CHART_COLORS[i % len(_CHART_COLORS)])
        else:  # bar
            bar_w = 0.7 / max(len(sites), 1)
            for i, site in enumerate(sites):
                vals    = [monthly_by_site[site].get(m) or 0 for m in months]
                offsets = [xi + (i - len(sites) / 2 + 0.5) * bar_w for xi in x]
