...
import { getDashboard, submitJustification as _submitJustificationRaw } from "../api/outliners";
...
// API 응답 래퍼 (기존 코드와 호환 — .data.data 구조로 맞춤)
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        // selectedIssues가 없으면 전체 이슈를 API에서 가져옴
        let issues = selectedIssues || [];
        if (issues.length === 0) {
          const { getIssues } = await import("../api/issues");
          const allRes = await getIssues();
          issues = (allRes?.data || []).map(i => ({ id: i.id, name: i.name }));
        }
        const tree   = [];
        ...
        if (hasV4) {
          try {
            ...
            // 재조회
            const refreshed = await _getDashboard({ limit: 5000 });
      ...
      <div style={s.body}>
        ...
        <main style={s.main}>
          ...
          <div style={s.issueBar}>
            ...
            <button
              style={s.prevBtn}
              onClick={() => navigate("/standard-data", { state: { issues: selectedIssues } })}
            >
              ← 표준화 데이터
            </button>
            <button
