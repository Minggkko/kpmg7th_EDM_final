...
// 사이트 이름 → 항상 동일한 색상 (연도에 관계없이 일관성 유지)
    ...
    <div>
      ...
      <ResponsiveContainer width="100%" height={200}>
        <BarChart data={data} margin={{ top: 4, right: 8, left: 0, bottom: 0 }}>
          ...
          {sites.map((site) => (
            <Bar key={site} dataKey={site} fill={siteColor(site)} radius={[3, 3, 0, 0]} maxBarSize={28} />
          ))}
        </BarChart>
      </ResponsiveContainer>
    </div>
            ...
            <div key={site} style={{ textAlign: 'center' }}>
              <PieChart width={120} height={120}>
                <Pie data={pieData} cx={55} cy={55} innerRadius={35} outerRadius={52} dataKey="value" startAngle={90} endAngle={-270}>
                  <Cell fill={siteColor(site)} />
                  <Cell fill="#E2E8F0" />
                </Pie>
                <Tooltip formatter={(v) => [v + '%']} />
              </PieChart>
              <div style={{ fontSize: 18, fontWeight: 700, color: siteColor(site), marginTop: -8 }}>
                {fmt(val)}%
              </div>
    ...
    <div>
      ...
      <ResponsiveContainer width="100%" height={200}>
        <LineChart data={data} margin={{ top: 4, right: 8, left: 0, bottom: 0 }}>
          ...
          {sites.length > 1 && <Legend iconSize={10} wrapperStyle={{ fontSize: 12 }} />}
          {sites.map((site) => (
            <Line key={site} type="monotone" dataKey={site} stroke={siteColor(site)} strokeWidth={2} dot={{ r: 3 }} activeDot={{ r: 5 }} />
          ))}
        </LineChart>
      </ResponsiveContainer>
    </div>
      ...
      <ResponsiveContainer width="100%" height={Math.max(80, data.length * 40)}>
        <BarChart data={data} layout="vertical" margin={{ top: 4, right: 40, left: 8, bottom: 0 }}>
          ...
          <Bar dataKey="value" radius={[0, 3, 3, 0]} maxBarSize={22}>
            {data.map((entry, i) => (
              <Cell key={i} fill={siteColor(entry.name)} />
            ))}
          </Bar>
        </BarChart>
      </ResponsiveContainer>
